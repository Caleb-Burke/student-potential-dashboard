...
def safe_geocode(address):
    try:
        geolocator = Nominatim(user_agent="student_mapper", timeout=2)
...
num_cols = list(metric_map.values())
...
with main_col:
    ...
    # ---------- Map ----------
    value_col = display_map[metric_map[color_metric]]
    ...
    for _, row in aggregated.iterrows():
        val = row[value_col]
        tooltip = f"{row['Neighborhood']}: {int(val)} {color_metric.lower()}"
        style_fn = lambda feature, val=val: {
            "fillColor": cmap(val),
            "color": "black",
            "weight": 0.5,
            "fillOpacity": 0.7
